#include "lvgl.h"

#ifndef LV_ATTRIBUTE_MEM_ALIGN
#define LV_ATTRIBUTE_MEM_ALIGN
#endif
#ifndef LV_ATTRIBUTE_IMG_BT_OUT_OK
#define LV_ATTRIBUTE_IMG_BT_OUT_OK
#endif
const LV_ATTRIBUTE_MEM_ALIGN LV_ATTRIBUTE_IMG_BT_OUT_OK uint8_t bt_out_ok_map[] = {
  0xff, 0xff, 0xff, 0xff, 	/*Color of index 0*/
  0x00, 0x00, 0x00, 0xff, 	/*Color of index 1*/

  , 
};

const lv_img_dsc_t bt_out_ok = {
  .header.always_zero = 0,
  .header.w = 33,
  .header.h = 33,
  .data_size = 174,
  .header.cf = LV_IMG_CF_INDEXED_1BIT,
  .data = bt_out_ok_map,
};
